...
var inputBoard = LibraryTools.Tools.GetInput(trial);
...
do
{
    try
    {
        ...
        GuardStep(board, position);
    }
    catch (InvalidOperationException)
    {
        var result = board.Cast<Cell>().Where(c => c.Visited == true).Count();
...
static List<Cell[]> GuardStep(List<Cell[]> board, (int row, int col) position)
{
    ...
    if ( nextPosition.Item1 >= 0 && nextPosition.Item1 < board.Count && nextPosition.Item2 >= 0 && nextPosition.Item2 < board[nextPosition.Item1].Length)
    {
        ...
        board[nextPosition.Item1][nextPosition.Item2] = guard;
    }
    board[position.row][position.col].Type = FieldType.Empty;
    return board;
}